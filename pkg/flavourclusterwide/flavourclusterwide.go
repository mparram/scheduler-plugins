...
func (f *FlavourClusterWide) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	// Find min and max pod counts for this flavour across all nodes
	minPods := -1
	maxPods := -1
	...
	podCount := 0
	if nodeCounts, exists := f.cache[nodeName]; exists {
		if count, exists := nodeCounts[flavour]; exists {
			podCount = count
		}
	}
	...
	// This ensures:
	// - Node with minPods gets the highest score (100)
